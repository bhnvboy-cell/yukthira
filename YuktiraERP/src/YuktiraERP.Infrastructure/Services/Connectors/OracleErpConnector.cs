...
public class OracleErpConnector : IConnector
{
    ...
    public string Description => "Connects to Oracle ERP Cloud via REST and SOAP APIs";
    public string[] SupportedAuthTypes => new[] { "Basic", "OAuth2", "Oracle_SSO" };
    public string[] SupportedActions => new[] { "GetEntity", "CreateEntity", "UpdateEntity", "GetAttachment", "SubmitESSJob", "InvokeSOAP" };

    public OracleErpConnector(HttpClient http) => _http = http;
    ...
    public async Task<TestConnectionResult> TestConnectionAsync(string baseUrl, string authType, Dictionary<string, string> authConfig, Dictionary<string, string> additionalConfig)
    {
        ...
        try
        {
            ...
            var resp = await _http.GetAsync(baseUrl.TrimEnd('/') + "/crmRestApi/resources/v1.0/metadata?mode=summary");
            sw.Stop();
            return new() { Success = resp.IsSuccessStatusCode, Message = resp.IsSuccessStatusCode ? "Oracle ERP reachable" : $"HTTP {resp.StatusCode}", ResponseTimeMs = (int)sw.ElapsedMilliseconds };
        }
        catch (Exception ex)
        {
            sw.Stop();
    ...
    public async Task<ConnectorActionResponse> ExecuteActionAsync(string baseUrl, string authType, Dictionary<string, string> authConfig, Dictionary<string, string> additionalConfig, string action, Dictionary<string, object>? parameters)
    {
        ...
        {
            ApplyAuth(authType, authConfig);
            var url = $"{baseUrl.TrimEnd('/')}/{(action == "InvokeSOAP" ? "soap" : $"crmRestApi/resources/v1.0/{action.ToLower()}")}";
    ...
    {
        ApplyAuth(authType, authConfig);
        var url = $"{baseUrl.TrimEnd('/')}/crmRestApi/resources/v1.0/{entityType}?limit=1000";
        if (lastSync.HasValue) url += $"&q=LastUpdateDate>{lastSync:yyyy-MM-dd}";
    ...
    public async Task<bool> PushDataAsync(string baseUrl, string authType, Dictionary<string, string> authConfig, Dictionary<string, string> additionalConfig, string entityType, List<Dictionary<string, object>> records)
    {
        ...
        {
            var resp = await _http.PostAsJsonAsync($"{baseUrl.TrimEnd('/')}/crmRestApi/resources/v1.0/{entityType}", rec);
