    ...
    where TEntity : class
{
    ...
    public async Task AddAsync(TEntity entity) { await _set.AddAsync(entity); await _context.SaveChangesAsync(); }
    public async Task UpdateAsync(TEntity entity) { _set.Update(entity); await _context.SaveChangesAsync(); }
    public async Task DeleteAsync(TKey id) { var e = await _set.FindAsync(id); if (e != null) { _set.Remove(e); await _context.SaveChangesAsync(); } }
