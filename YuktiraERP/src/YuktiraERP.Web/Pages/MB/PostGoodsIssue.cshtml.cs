...
[Authorize]
public class PostGoodsIssueModel : PageModel
{
    private readonly IInventoryMovementService _service;
    private readonly ITenantContext _tenant;

    public PostGoodsIssueModel(IInventoryMovementService service, ITenantContext tenant)
    {
        _service = service;
        _tenant = tenant;
    }
    ...
    [BindProperty]
    public int MovementType { get; set; } = 201;
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        Result = await _service.PostGoodsIssueAsync(dto, _tenant.TenantId, userId);
        if (Result.Success)
